...
const char* smartcard_get_ioctl_string(UINT32 ioControlCode, BOOL funcName)
{
	switch (ioControlCode)
	{
		...
		case SCARD_IOCTL_INTRODUCEREADERGROUPA:
			return funcName ? "SCardIntroduceReaderGroupA" :
			       "SCARD_IOCTL_INTRODUCEREADERGROUPA";

		case SCARD_IOCTL_INTRODUCEREADERGROUPW:
			return funcName ? "SCardIntroduceReaderGroupW" :
		...
		case SCARD_IOCTL_REMOVEREADERFROMGROUPA:
			return funcName ? "SCardRemoveReaderFromGroupA" :
			       "SCARD_IOCTL_REMOVEREADERFROMGROUPA";

		case SCARD_IOCTL_REMOVEREADERFROMGROUPW:
			return funcName ? "SCardRemoveReaderFromGroupW" :
		...
		case SCARD_IOCTL_RELEASESTARTEDEVENT:
			return funcName ? "SCardReleaseStartedEvent" :
...
static UINT32 smartcard_EstablishContext_Decode(SMARTCARD_DEVICE* smartcard,
...
static UINT32 smartcard_EstablishContext_Call(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation)
{
	UINT32 status;
	...
	status = ret.ReturnCode = SCardEstablishContext(call->dwScope, NULL, NULL,
	                          &hContext);
...
static UINT32 smartcard_ReleaseContext_Decode(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation)
{
	UINT32 status;
	Context_Call* call;
...
static UINT32 smartcard_ReleaseContext_Call(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation)
{
	UINT32 status;
	Long_Return ret;
	...
	if (ret.ReturnCode == SCARD_S_SUCCESS)
	{
		...
		pContext = (SMARTCARD_CONTEXT*) ListDictionary_Remove(
		               smartcard->rgSCardContextList, key);
...
static UINT32 smartcard_IsValidContext_Decode(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation)
{
	UINT32 status;
	Context_Call* call;
...
static UINT32 smartcard_IsValidContext_Call(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation)
{
	UINT32 status;
	Long_Return ret;
...
static UINT32 smartcard_ListReaderGroupsW_Decode(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation)
{
	UINT32 status;
	IRP* irp = operation->irp;
	...
	if ((status = smartcard_unpack_list_readers_call(smartcard, irp->input,
	              operation->call)))
		WLog_ERR(TAG, "smartcard_unpack_list_readers_call failed with error %lu",
		         status);

	smartcard_trace_list_readers_call(smartcard, operation->call, FALSE);
	operation->hContext = smartcard_scard_context_native_from_redir(smartcard,
	                      &(call->hContext));
	return status;
}

static UINT32 smartcard_ListReaderGroupsA_Decode(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation)
{
	UINT32 status;
	ListReaderGroups_Call* call;
...
static UINT32 smartcard_ListReaderGroupsA_Call(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation)
{
	UINT32 status;
	...
	status = ret.ReturnCode = SCardListReaderGroupsA(operation->hContext,
	                          (LPSTR) &mszGroups, &cchGroups);
...
static UINT32 smartcard_ListReaderGroupsW_Call(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation)
{
	UINT32 status;
	ListReaderGroups_Return ret;
	...
	IRP* irp = operation->irp;
	cchGroups = SCARD_AUTOALLOCATE;
	status = ret.ReturnCode = SCardListReaderGroupsW(operation->hContext,
...
static UINT32 smartcard_ListReadersA_Decode(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation)
{
	UINT32 status;
                                        ...
                                        SMARTCARD_OPERATION* operation)
{
	...
	status = ret.ReturnCode = SCardListReadersA(operation->hContext,
	...
	{
		WLog_ERR(TAG, "smartcard_pack_list_readers_return failed with error %lu",
		         status);
		return status;
	}
...
static UINT32 smartcard_ListReadersW_Decode(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation)
{
	UINT32 status;
	ListReaders_Call* call;
...
static UINT32 smartcard_ListReadersW_Call(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation)
{
	UINT32 status;
	ListReaders_Return ret;
...
static UINT32 smartcard_GetStatusChangeA_Decode(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation)
{
	LONG status;
	...
	if ((status = smartcard_unpack_get_status_change_a_call(smartcard, irp->input,
	              call)))
	{
		WLog_ERR(TAG, "smartcard_unpack_get_status_change_a_call failed with error %lu",
		         status);
		return status;
	}
...
static UINT32 smartcard_GetStatusChangeA_Call(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation)
{
	LONG status;
	UINT32 index;
	...
	if (ret.cReaders > 0)
		ret.rgReaderStates = (ReaderState_Return*) calloc(ret.cReaders,
		                     sizeof(ReaderState_Return));
	...
	{
		ret.rgReaderStates[index].dwCurrentState =
		    call->rgReaderStates[index].dwCurrentState;
		ret.rgReaderStates[index].dwEventState =
		    call->rgReaderStates[index].dwEventState;
		ret.rgReaderStates[index].cbAtr = call->rgReaderStates[index].cbAtr;
		CopyMemory(&(ret.rgReaderStates[index].rgbAtr),
		           &(call->rgReaderStates[index].rgbAtr), 32);
	}

	smartcard_trace_get_status_change_return(smartcard, &ret, FALSE);

	if ((status = smartcard_pack_get_status_change_return(smartcard, irp->output,
	              &ret)))
	{
		WLog_ERR(TAG, "smartcard_pack_get_status_change_return failed with error %lu",
		         status);
		return status;
	}
...
static UINT32 smartcard_GetStatusChangeW_Decode(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation)
{
	LONG status;
	...
	if ((status = smartcard_unpack_get_status_change_w_call(smartcard, irp->input,
	              call)))
		WLog_ERR(TAG, "smartcard_unpack_get_status_change_w_call failed with error %lu",
		         status);
...
static UINT32 smartcard_GetStatusChangeW_Call(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation)
{
	LONG status;
	UINT32 index;
	...
	status = ret.ReturnCode = SCardGetStatusChangeW(operation->hContext,
	                          call->dwTimeOut, call->rgReaderStates, call->cReaders);
...
static UINT32 smartcard_Cancel_Decode(SMARTCARD_DEVICE* smartcard,
                                      SMARTCARD_OPERATION* operation)
{
	LONG status;
	Context_Call* call;
...
static UINT32 smartcard_Cancel_Call(SMARTCARD_DEVICE* smartcard,
                                    SMARTCARD_OPERATION* operation)
{
	LONG status;
	Long_Return ret;
...
static UINT32 smartcard_ConnectA_Decode(SMARTCARD_DEVICE* smartcard,
                                        SMARTCARD_OPERATION* operation)
{
	LONG status;
	ConnectA_Call* call;
...
static UINT32 smartcard_ConnectA_Call(SMARTCARD_DEVICE* smartcard,
                                      SMARTCARD_OPERATION* operation)
{
	LONG status;
	SCARDHANDLE hCard = 0;
	...
	status = ret.ReturnCode = SCardConnectA(operation->hContext,
	                                        (char*) call->szReader, call->Common.dwShareMode,
	                                        call->Common.dwPreferredProtocols, &hCard, &ret.dwActiveProtocol);
	smartcard_scard_context_native_to_redir(smartcard, &(ret.hContext),
	                                        operation->hContext);
...
static UINT32 smartcard_ConnectW_Decode(SMARTCARD_DEVICE* smartcard,
                                        SMARTCARD_OPERATION* operation)
{
	LONG status;
	ConnectW_Call* call;
...
static UINT32 smartcard_ConnectW_Call(SMARTCARD_DEVICE* smartcard,
                                      SMARTCARD_OPERATION* operation)
{
	LONG status;
	SCARDHANDLE hCard = 0;
	...
	status = ret.ReturnCode = SCardConnectW(operation->hContext,
	                                        (WCHAR*) call->szReader, call->Common.dwShareMode,
	                                        call->Common.dwPreferredProtocols, &hCard, &ret.dwActiveProtocol);
	smartcard_scard_context_native_to_redir(smartcard, &(ret.hContext),
	                                        operation->hContext);
	smartcard_scard_handle_native_to_redir(smartcard, &(ret.hCard), hCard);
...
static UINT32 smartcard_Reconnect_Decode(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation)
{
	LONG status;
	Reconnect_Call* call;
	...
	operation->hContext = smartcard_scard_context_native_from_redir(smartcard,
	                      &(call->hContext));
	operation->hCard = smartcard_scard_handle_native_from_redir(smartcard,
	                   &(call->hCard));
	return status;
}

static UINT32 smartcard_Reconnect_Call(SMARTCARD_DEVICE* smartcard,
                                       SMARTCARD_OPERATION* operation)
{
	LONG status;
...
static UINT32 smartcard_Disconnect_Decode(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation)
{
	LONG status;
	...
	status = smartcard_unpack_hcard_and_disposition_call(smartcard, irp->input,
	         call);
...
static UINT32 smartcard_Disconnect_Call(SMARTCARD_DEVICE* smartcard,
                                        SMARTCARD_OPERATION* operation)
{
	LONG status;
	Long_Return ret;
	HCardAndDisposition_Call* call = operation->call;
	status = ret.ReturnCode = SCardDisconnect(operation->hCard,
	                          call->dwDisposition);
...
static UINT32 smartcard_BeginTransaction_Decode(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation)
{
	LONG status;
	HCardAndDisposition_Call* call;
	...
	if ((status = smartcard_unpack_hcard_and_disposition_call(smartcard, irp->input,
	              call)))
		WLog_ERR(TAG,
		         "smartcard_unpack_hcard_and_disposition_call failed with error %lu", status);
...
static UINT32 smartcard_BeginTransaction_Call(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation)
{
	LONG status;
	Long_Return ret;
...
static UINT32 smartcard_EndTransaction_Decode(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation)
{
	LONG status;
	HCardAndDisposition_Call* call;
...
static UINT32 smartcard_EndTransaction_Call(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation)
{
	LONG status;
	Long_Return ret;
	HCardAndDisposition_Call* call = operation->call;
	status = ret.ReturnCode = SCardEndTransaction(operation->hCard,
	                          call->dwDisposition);
	smartcard_trace_long_return(smartcard, &ret, "EndTransaction");
	return ret.ReturnCode;
}

static UINT32 smartcard_State_Decode(SMARTCARD_DEVICE* smartcard,
                                     SMARTCARD_OPERATION* operation)
{
	LONG status;
	State_Call* call;
...
static UINT32 smartcard_State_Call(SMARTCARD_DEVICE* smartcard,
                                   SMARTCARD_OPERATION* operation)
{
	LONG status;
	State_Return ret;
	...
	status = ret.ReturnCode = SCardState(operation->hCard, &ret.dwState,
	                                     &ret.dwProtocol, (BYTE*) &ret.rgAtr, &ret.cbAtrLen);
...
static DWORD smartcard_StatusA_Decode(SMARTCARD_DEVICE* smartcard,
                                      SMARTCARD_OPERATION* operation)
{
	LONG status;
	Status_Call* call;
...
static DWORD smartcard_StatusA_Call(SMARTCARD_DEVICE* smartcard,
                                    SMARTCARD_OPERATION* operation)
{
	LONG status;
	Status_Return ret = { 0 };
	...
	status = ret.ReturnCode = SCardStatusA(operation->hCard,
	                                       (LPSTR) &mszReaderNames, &cchReaderLen,
...
static DWORD smartcard_StatusW_Decode(SMARTCARD_DEVICE* smartcard,
                                      SMARTCARD_OPERATION* operation)
{
	LONG status;
	Status_Call* call;
...
static DWORD smartcard_StatusW_Call(SMARTCARD_DEVICE* smartcard,
                                    SMARTCARD_OPERATION* operation)
{
	LONG status;
	Status_Return ret;
...
static UINT32 smartcard_Transmit_Decode(SMARTCARD_DEVICE* smartcard,
                                        SMARTCARD_OPERATION* operation)
{
	LONG status;
	Transmit_Call* call;
...
static UINT32 smartcard_Transmit_Call(SMARTCARD_DEVICE* smartcard,
                                      SMARTCARD_OPERATION* operation)
{
	LONG status;
	Transmit_Return ret;
	...
	status = ret.ReturnCode = SCardTransmit(operation->hCard, call->pioSendPci,
	                                        call->pbSendBuffer,
...
static UINT32 smartcard_Control_Decode(SMARTCARD_DEVICE* smartcard,
                                       SMARTCARD_OPERATION* operation)
{
	LONG status;
	Control_Call* call;
...
static UINT32 smartcard_Control_Call(SMARTCARD_DEVICE* smartcard,
                                     SMARTCARD_OPERATION* operation)
{
	LONG status;
	Control_Return ret;
...
static UINT32 smartcard_GetAttrib_Decode(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation)
{
	LONG status;
	GetAttrib_Call* call;
...
static UINT32 smartcard_GetAttrib_Call(SMARTCARD_DEVICE* smartcard,
                                       SMARTCARD_OPERATION* operation)
{
	LONG status;
	DWORD cbAttrLen;
...
static UINT32 smartcard_AccessStartedEvent_Decode(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation)
{
	Long_Call* call;
	IRP* irp = operation->irp;
...
static UINT32 smartcard_AccessStartedEvent_Call(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation)
{
	UINT32 status;
	Long_Return ret;
...
static UINT32 smartcard_LocateCardsByATRA_Decode(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation)
{
	LONG status;
	...
	if ((status = smartcard_unpack_locate_cards_by_atr_a_call(smartcard, irp->input,
	              call)))
		WLog_ERR(TAG,
		         "smartcard_unpack_locate_cards_by_atr_a_call failed with error %lu", status);
        ...
        SMARTCARD_OPERATION* operation)
{
	...
	states = (LPSCARD_READERSTATEA) calloc(call->cReaders,
	...
	status = ret.ReturnCode = SCardGetStatusChangeA(operation->hContext, 0x000001F4,
	                          states, call->cReaders);
        ...
        SMARTCARD_OPERATION* operation)
{
	...
	if (Stream_Length(irp->input) != (Stream_GetPosition(irp->input) +
	                                  inputBufferLength))
	{
	...
	if (((size_t) Stream_GetPosition(irp->input)) < Stream_Length(irp->input))
	{
		...
		WLog_WARN(TAG,
		          ...
		          (int) Stream_GetPosition(irp->input), (int) Stream_Length(irp->input),
		          difference);
                                       ...
                                       SMARTCARD_OPERATION* operation)
{
	...
	Stream_Seek(irp->output,
	            SMARTCARD_COMMON_TYPE_HEADER_LENGTH); /* CommonTypeHeader (8 bytes) */
	Stream_Seek(irp->output,
	...
	if ((ioControlCode != SCARD_IOCTL_ACCESSSTARTEDEVENT) &&
	    (ioControlCode != SCARD_IOCTL_RELEASESTARTEDEVENT))
	{
		offset = (RDPDR_DEVICE_IO_RESPONSE_LENGTH +
	...
	outputBufferLength = Stream_Length(irp->output) -
	                     RDPDR_DEVICE_IO_RESPONSE_LENGTH - 4;
	...
	{
		WLog_ERR(TAG, "smartcard_pack_common_type_header failed with error %lu",
		         result);
		return result;
	}
	...
	{
		WLog_ERR(TAG, "smartcard_pack_private_type_header failed with error %lu",
		         result);
		return result;
	}
