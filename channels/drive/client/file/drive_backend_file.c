...
static WCHAR* drive_file_combine_fullpath(const WCHAR* base_path, const WCHAR* path)
{
	...
	const size_t PathWCharLength = _wcsnlen(path, MAX_PATH);
                                           ...
                                           size_t numCharacters)
{
	...
	context->file_handle = FindFirstFileW(query, &context->find_data);
	if (context->file_handle == INVALID_HANDLE_VALUE)
		return NULL;
...
static bool drive_update_path(rdpDriveContext* context)
{
	...
	context->fullpath = drive_file_combine_fullpath(context->base_path, context->filename);
...
static const BY_HANDLE_FILE_INFORMATION* drive_getFileAttributeData(rdpDriveContext* context)
{
	...
	if (context->file_handle != INVALID_HANDLE_VALUE)
	{
		if (GetFileInformationByHandle(context->file_handle, &context->file_by_handle))
			goto out;
	}
...
static bool drive_move(rdpDriveContext* context, const WCHAR* newName, size_t numCharacters)
{
	...
	WCHAR* newpath = drive_file_combine_fullpath(context->base_path, newName);
	if (!newpath)
		return false;
	const BOOL res = MoveFileW(context->fullpath, newpath);
	free(newpath);
	if (!res)
		return false;

	return drive_setPath(context, context->base_path, newName);
}

static const rdpDriveDriver s_driver = { .createDirectory = drive_createDirectory,
