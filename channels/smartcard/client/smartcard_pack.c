...
UINT32 smartcard_unpack_common_type_header(SMARTCARD_DEVICE* smartcard,
        wStream* s)
{
	UINT8 version;
	UINT32 filler;
	...
	{
		WLog_WARN(TAG, "Unsupported CommonTypeHeader CommonHeaderLength %d",
		          commonHeaderLength);
		return STATUS_INVALID_PARAMETER;
	}
...
UINT32 smartcard_pack_common_type_header(SMARTCARD_DEVICE* smartcard,
        wStream* s)
{
...
UINT32 smartcard_unpack_private_type_header(SMARTCARD_DEVICE* smartcard,
        wStream* s)
{
	UINT32 filler;
	...
	{
		WLog_WARN(TAG,
		          "PrivateTypeHeader ObjectBufferLength mismatch: Actual: %d, Expected: %d",
...
UINT32 smartcard_pack_private_type_header(SMARTCARD_DEVICE* smartcard,
...
UINT32 smartcard_unpack_read_size_align(SMARTCARD_DEVICE* smartcard, wStream* s,
...
UINT32 smartcard_pack_write_size_align(SMARTCARD_DEVICE* smartcard, wStream* s,
                                       UINT32 size, UINT32 alignment)
{
	UINT32 pad;
	pad = size;
...
SCARDCONTEXT smartcard_scard_context_native_from_redir(SMARTCARD_DEVICE*
        smartcard, REDIR_SCARDCONTEXT* context)
{
...
SCARDHANDLE smartcard_scard_handle_native_from_redir(SMARTCARD_DEVICE*
        smartcard, REDIR_SCARDHANDLE* handle)
{
	SCARDHANDLE hCard = 0;
...
UINT32 smartcard_unpack_redir_scard_context(SMARTCARD_DEVICE* smartcard,
        wStream* s, REDIR_SCARDCONTEXT* context)
{
	...
	if ((context->cbContext != 0) && (context->cbContext != 4)
	    && (context->cbContext != 8))
	{
		WLog_WARN(TAG, "REDIR_SCARDCONTEXT length is not 0, 4 or 8: %d",
		          context->cbContext);
		return STATUS_INVALID_PARAMETER;
	}
	...
	{
		WLog_WARN(TAG,
		          "REDIR_SCARDCONTEXT cbContext (%d) pbContextNdrPtr (%d) inconsistency",
...
UINT32 smartcard_pack_redir_scard_context(SMARTCARD_DEVICE* smartcard,
        wStream* s, REDIR_SCARDCONTEXT* context)
{
	UINT32 pbContextNdrPtr;
...
UINT32 smartcard_unpack_redir_scard_context_ref(SMARTCARD_DEVICE* smartcard,
        wStream* s, REDIR_SCARDCONTEXT* context)
{
	UINT32 length;
	...
	if ((context->cbContext != 0) && (context->cbContext != 4)
	    && (context->cbContext != 8))
	{
		WLog_WARN(TAG, "REDIR_SCARDCONTEXT length is not 4 or 8: %d",
		          context->cbContext);
		return STATUS_INVALID_PARAMETER;
	}
...
UINT32 smartcard_pack_redir_scard_context_ref(SMARTCARD_DEVICE* smartcard,
        wStream* s, REDIR_SCARDCONTEXT* context)
{
...
UINT32 smartcard_unpack_redir_scard_handle(SMARTCARD_DEVICE* smartcard,
        wStream* s, REDIR_SCARDHANDLE* handle)
{
...
UINT32 smartcard_pack_redir_scard_handle(SMARTCARD_DEVICE* smartcard,
        wStream* s, REDIR_SCARDHANDLE* handle)
{
	UINT32 pbHandleNdrPtr;
...
UINT32 smartcard_unpack_redir_scard_handle_ref(SMARTCARD_DEVICE* smartcard,
        wStream* s, REDIR_SCARDHANDLE* handle)
{
	UINT32 length;
...
UINT32 smartcard_pack_redir_scard_handle_ref(SMARTCARD_DEVICE* smartcard,
        wStream* s, REDIR_SCARDHANDLE* handle)
{
...
UINT32 smartcard_unpack_establish_context_call(SMARTCARD_DEVICE* smartcard,
        wStream* s, EstablishContext_Call* call)
{
	if (Stream_GetRemainingLength(s) < 4)
	{
...
UINT32 smartcard_pack_establish_context_return(SMARTCARD_DEVICE* smartcard,
        wStream* s, EstablishContext_Return* ret)
{
	UINT32 status;

	if ((status = smartcard_pack_redir_scard_context(smartcard, s,
	              &(ret->hContext))))
	{
		WLog_ERR(TAG, "smartcard_pack_redir_scard_context failed with error %lu",
		         status);
		return status;
	}

	if ((status = smartcard_pack_redir_scard_context_ref(smartcard, s,
	              &(ret->hContext))))
		WLog_ERR(TAG, "smartcard_pack_redir_scard_context_ref failed with error %lu",
		         status);
...
UINT32 smartcard_unpack_context_call(SMARTCARD_DEVICE* smartcard, wStream* s,
                                     Context_Call* call)
{
	UINT32 status;

	if ((status = smartcard_unpack_redir_scard_context(smartcard, s,
	              &(call->hContext))))
	{
		WLog_ERR(TAG, "smartcard_unpack_redir_scard_context failed with error %lu",
		         status);
		return status;
	}
	...
	if ((status = smartcard_unpack_redir_scard_context_ref(smartcard, s,
	              &(call->hContext))))
		WLog_ERR(TAG, "smartcard_unpack_redir_scard_context_ref failed with error %lu",
		         status);
...
void smartcard_trace_context_call(SMARTCARD_DEVICE* smartcard,
                                  Context_Call* call, const char* name)
{
	BYTE* pb;
...
void smartcard_trace_long_return(SMARTCARD_DEVICE* smartcard, Long_Return* ret,
                                 const char* name)
{
	if (!WLog_IsLevelActive(WLog_Get(TAG), WLOG_DEBUG))
		return;
...
UINT32 smartcard_unpack_list_reader_groups_call(SMARTCARD_DEVICE* smartcard,
        wStream* s, ListReaderGroups_Call* call)
{
	UINT32 status;
	...
	status = smartcard_unpack_redir_scard_context_ref(smartcard, s,
	         &(call->hContext));
...
UINT32 smartcard_pack_list_reader_groups_return(SMARTCARD_DEVICE* smartcard,
        wStream* s, ListReaderGroups_Return* ret)
{
	UINT32 mszNdrPtr;
...
UINT32 smartcard_unpack_list_readers_call(SMARTCARD_DEVICE* smartcard,
        wStream* s, ListReaders_Call* call)
{
	UINT32 status;
	UINT32 count;
	...
	Stream_Read_UINT32(s,
	                   call->fmszReadersIsNULL); /* fmszReadersIsNULL (4 bytes) */
                                       ...
                                       ListReaders_Call* call, BOOL unicode)
{
	...
	if (unicode)
		ConvertFromUnicode(CP_UTF8, 0, (WCHAR*) call->mszGroups, call->cBytes / 2,
		                   &mszGroupsA, 0, NULL, NULL);
...
UINT32 smartcard_pack_list_readers_return(SMARTCARD_DEVICE* smartcard,
        wStream* s, ListReaders_Return* ret)
{
	UINT32 mszNdrPtr;
	UINT32 error;
...
void smartcard_trace_list_readers_return(SMARTCARD_DEVICE* smartcard,
        ListReaders_Return* ret, BOOL unicode)
{
	int index;
	size_t length;
	char* mszA = NULL;
                                     ...
                                     Connect_Common* common)
{
	...
	Stream_Read_UINT32(s,
	                   common->dwPreferredProtocols); /* dwPreferredProtocols (4 bytes) */
	return SCARD_S_SUCCESS;
}

UINT32 smartcard_unpack_connect_a_call(SMARTCARD_DEVICE* smartcard, wStream* s,
                                       ConnectA_Call* call)
{
	UINT32 status;
	UINT32 count;
                                    ...
                                    ConnectA_Call* call)
{
	...
	WLog_DBG(TAG,
	         "szReader: %s dwShareMode: %s (0x%08X) dwPreferredProtocols: %s (0x%08X)",
...
UINT32 smartcard_unpack_connect_w_call(SMARTCARD_DEVICE* smartcard, wStream* s,
                                       ConnectW_Call* call)
{
	UINT32 status;
	UINT32 count;
...
UINT32 smartcard_pack_connect_return(SMARTCARD_DEVICE* smartcard, wStream* s,
                                     Connect_Return* ret)
{
	UINT32 status;
	...
	{
		WLog_ERR(TAG, "smartcard_pack_redir_scard_handle failed with error %lu",
		         status);
		return status;
	}
	...
	if ((status = smartcard_pack_redir_scard_handle_ref(smartcard, s,
	              &(ret->hCard))))
		WLog_ERR(TAG, "smartcard_pack_redir_scard_handle_ref failed with error %lu",
		         status);
...
UINT32 smartcard_unpack_reconnect_call(SMARTCARD_DEVICE* smartcard, wStream* s,
                                       Reconnect_Call* call)
{
	UINT32 status;
	...
	if ((status = smartcard_unpack_redir_scard_handle(smartcard, s,
	              &(call->hCard))))
	{
		WLog_ERR(TAG, "smartcard_unpack_redir_scard_handle failed with error %lu",
		         status);
		return status;
	}
	...
	Stream_Read_UINT32(s,
	                   call->dwPreferredProtocols); /* dwPreferredProtocols (4 bytes) */
	...
	if ((status = smartcard_unpack_redir_scard_handle_ref(smartcard, s,
	              &(call->hCard))))
		WLog_ERR(TAG, "smartcard_unpack_redir_scard_handle_ref failed with error %lu",
		         status);
...
UINT32 smartcard_pack_reconnect_return(SMARTCARD_DEVICE* smartcard, wStream* s,
...
UINT32 smartcard_unpack_hcard_and_disposition_call(SMARTCARD_DEVICE* smartcard,
        wStream* s, HCardAndDisposition_Call* call)
{
	UINT32 status;
...
UINT32 smartcard_unpack_get_status_change_a_call(SMARTCARD_DEVICE* smartcard,
        wStream* s, GetStatusChangeA_Call* call)
{
	UINT32 index;
	UINT32 count;
	UINT32 status;
	...
	Stream_Read_UINT32(s,
	                   rgReaderStatesNdrPtr); /* rgReaderStatesNdrPtr (4 bytes) */
	...
	if (call->cReaders > 0)
	{
		call->rgReaderStates = (LPSCARD_READERSTATEA) calloc(call->cReaders,
		                       sizeof(SCARD_READERSTATEA));

		if (!call->rgReaderStates)
		{
			WLog_WARN(TAG,
			          "GetStatusChangeA_Call out of memory error (call->rgReaderStates)");
			return STATUS_NO_MEMORY;
		}
		...
		for (index = 0; index < call->cReaders; index++)
		{
			...
			Stream_Read_UINT32(s,
			                   readerState->dwCurrentState); /* dwCurrentState (4 bytes) */
...
UINT32 smartcard_unpack_get_status_change_w_call(SMARTCARD_DEVICE* smartcard,
        wStream* s, GetStatusChangeW_Call* call)
{
	UINT32 index;
	UINT32 count;
	UINT32 status;
	...
	if (call->cReaders > 0)
	{
		call->rgReaderStates = (LPSCARD_READERSTATEW) calloc(call->cReaders,
		                       sizeof(SCARD_READERSTATEW));

		if (!call->rgReaderStates)
		{
			WLog_WARN(TAG,
			          "GetStatusChangeW_Call out of memory error (call->rgReaderStates)");
			return STATUS_NO_MEMORY;
		}
        ...
        GetStatusChangeW_Call* call)
{
	...
	for (index = 0; index < call->cReaders; index++)
	{
		char* szReaderA = NULL;
		readerState = &call->rgReaderStates[index];
		ConvertFromUnicode(CP_UTF8, 0, readerState->szReader, -1, &szReaderA, 0, NULL,
		                   NULL);
		WLog_DBG(TAG, "\t[%d]: szReader: %s cbAtr: %d",
...
UINT32 smartcard_pack_get_status_change_return(SMARTCARD_DEVICE* smartcard,
        wStream* s, GetStatusChange_Return* ret)
{
	UINT32 index;
	...
	for (index = 0; index < ret->cReaders; index++)
	{
		...
		Stream_Write_UINT32(s,
		                    rgReaderState->dwCurrentState); /* dwCurrentState (4 bytes) */
		Stream_Write_UINT32(s,
		                    rgReaderState->dwEventState); /* dwEventState (4 bytes) */
        ...
        GetStatusChange_Return* ret, BOOL unicode)
{
	...
	for (index = 0; index < ret->cReaders; index++)
	{
		...
		rgbAtr = winpr_BinToHexString((BYTE*) & (rgReaderState->rgbAtr),
		                              rgReaderState->cbAtr, FALSE);
		WLog_DBG(TAG, "\t[%d]: dwCurrentState: %s (0x%08X)",
...
UINT32 smartcard_unpack_state_call(SMARTCARD_DEVICE* smartcard, wStream* s,
                                   State_Call* call)
{
	UINT32 status;
...
UINT32 smartcard_pack_state_return(SMARTCARD_DEVICE* smartcard, wStream* s,
                                   State_Return* ret)
{
	UINT32 status;
...
UINT32 smartcard_unpack_status_call(SMARTCARD_DEVICE* smartcard, wStream* s,
                                    Status_Call* call)
{
	UINT32 status;
	...
	Stream_Read_UINT32(s,
	                   call->fmszReaderNamesIsNULL); /* fmszReaderNamesIsNULL (4 bytes) */
...
UINT32 smartcard_pack_status_return(SMARTCARD_DEVICE* smartcard, wStream* s,
                                    Status_Return* ret)
{
	UINT32 status;
...
void smartcard_trace_status_return(SMARTCARD_DEVICE* smartcard,
                                   Status_Return* ret, BOOL unicode)
{
	int index;
	size_t length;
	char* pbAtr = NULL;
...
UINT32 smartcard_unpack_get_attrib_call(SMARTCARD_DEVICE* smartcard, wStream* s,
                                        GetAttrib_Call* call)
{
	UINT32 status;
...
UINT32 smartcard_pack_get_attrib_return(SMARTCARD_DEVICE* smartcard, wStream* s,
...
void smartcard_trace_get_attrib_return(SMARTCARD_DEVICE* smartcard,
                                       GetAttrib_Return* ret, DWORD dwAttrId)
{
	if (!WLog_IsLevelActive(WLog_Get(TAG), WLOG_DEBUG))
		return;
...
UINT32 smartcard_unpack_control_call(SMARTCARD_DEVICE* smartcard, wStream* s,
                                     Control_Call* call)
{
	UINT32 status;
	UINT32 length;
	...
	Stream_Read_UINT32(s,
	                   call->fpvOutBufferIsNULL); /* fpvOutBufferIsNULL (4 bytes) */
                                  ...
                                  Control_Call* call)
{
	...
	if (call->pvInBuffer)
	{
		char* szInBuffer = winpr_BinToHexString(call->pvInBuffer, call->cbInBufferSize,
		                                        TRUE);
...
UINT32 smartcard_pack_control_return(SMARTCARD_DEVICE* smartcard, wStream* s,
                                     Control_Return* ret)
{
	UINT32 error;
	...
	if (ret->cbOutBufferSize > 0)
	{
		...
		if ((error = smartcard_pack_write_size_align(smartcard, s, ret->cbOutBufferSize,
		             4)))
		{
			WLog_ERR(TAG, "smartcard_pack_write_size_align failed with error %lu", error);
			return error;
                                    ...
                                    Control_Return* ret)
{
	...
	if (ret->pvOutBuffer)
	{
		char* szOutBuffer = winpr_BinToHexString(ret->pvOutBuffer, ret->cbOutBufferSize,
		                    TRUE);
...
UINT32 smartcard_unpack_transmit_call(SMARTCARD_DEVICE* smartcard, wStream* s,
                                      Transmit_Call* call)
{
	UINT32 status;
	UINT32 length;
	...
	Stream_Read_UINT32(s,
	                   call->fpbRecvBufferIsNULL); /* fpbRecvBufferIsNULL (4 bytes) */
	Stream_Read_UINT32(s, call->cbRecvLength); /* cbRecvLength (4 bytes) */

	if (ioSendPci.cbExtraBytes > 1024)
	{
		WLog_WARN(TAG,
		          "Transmit_Call ioSendPci.cbExtraBytes is out of bounds: %d (max: %d)",
	...
	if (pbExtraBytesNdrPtr)
	{
		...
		call->pioSendPci->cbPciLength = (DWORD)(ioSendPci.cbExtraBytes + sizeof(
		        SCARD_IO_REQUEST));
	...
	if (pioRecvPciNdrPtr)
	{
		...
		if (pbExtraBytesNdrPtr)
		{
			...
			{
				WLog_WARN(TAG,
				          "Transmit_Call ioRecvPci.cbExtraBytes is out of bounds: %d (max: %d)",
			...
			call->pioRecvPci = (LPSCARD_IO_REQUEST) malloc(sizeof(SCARD_IO_REQUEST) +
			...
			call->pioRecvPci->cbPciLength = (DWORD)(ioRecvPci.cbExtraBytes + sizeof(
			        SCARD_IO_REQUEST));
                                   ...
                                   Transmit_Call* call)
{
	...
	if (call->pioSendPci)
	{
		cbExtraBytes = (UINT32)(call->pioSendPci->cbPciLength - sizeof(
		                            SCARD_IO_REQUEST));
		pbExtraBytes = &((BYTE*) call->pioSendPci)[sizeof(SCARD_IO_REQUEST)];
	...
	if (call->pbSendBuffer)
	{
		char* szSendBuffer = winpr_BinToHexString(call->pbSendBuffer,
		                     call->cbSendLength, TRUE);
	...
	if (call->pioRecvPci)
	{
		cbExtraBytes = (UINT32)(call->pioRecvPci->cbPciLength - sizeof(
		                            SCARD_IO_REQUEST));
		pbExtraBytes = &((BYTE*) call->pioRecvPci)[sizeof(SCARD_IO_REQUEST)];
...
UINT32 smartcard_pack_transmit_return(SMARTCARD_DEVICE* smartcard, wStream* s,
                                      Transmit_Return* ret)
{
	UINT32 cbExtraBytes;
	BYTE* pbExtraBytes;
	...
	if (pioRecvPciNdrPtr)
	{
		cbExtraBytes = (UINT32)(ret->pioRecvPci->cbPciLength - sizeof(
		                            SCARD_IO_REQUEST));
	...
	if (pbRecvBufferNdrPtr)
	{
		...
		if ((error = smartcard_pack_write_size_align(smartcard, s, ret->cbRecvLength,
		             4)))
		{
			WLog_ERR(TAG, "smartcard_pack_write_size_align failed with error %lu!", error);
			return error;
                                     ...
                                     Transmit_Return* ret)
{
	...
	if (ret->pbRecvBuffer)
	{
		char* szRecvBuffer = winpr_BinToHexString(ret->pbRecvBuffer, ret->cbRecvLength,
		                     TRUE);
...
UINT32 smartcard_unpack_locate_cards_by_atr_a_call(SMARTCARD_DEVICE* smartcard,
        wStream* s, LocateCardsByATRA_Call* call)
{
	UINT32 index;
	UINT32 count;
	LONG status;
	...
	if (call->cReaders > 0)
	{
		...
		{
			WLog_WARN(TAG,
			          "LocateCardsByATRA_Call out of memory error (call->rgReaderStates)");
			return STATUS_NO_MEMORY;
		}
        ...
        LocateCardsByATRA_Call* call)
{
	...
	for (index = 0; index < call->cReaders; index++)
	{
		...
		szCurrentState = SCardGetReaderStateString(readerState->dwCurrentState);
		szEventState = SCardGetReaderStateString(readerState->dwEventState);
		rgbAtr = winpr_BinToHexString((BYTE*) & (readerState->rgbAtr),
		                              readerState->cbAtr, FALSE);
		WLog_DBG(TAG, "\t[%d]: dwCurrentState: %s (0x%08X)",
		         index, szCurrentState, readerState->dwCurrentState);
		WLog_DBG(TAG, "\t[%d]: dwEventState: %s (0x%08X)",
		         index, szEventState, readerState->dwEventState);
