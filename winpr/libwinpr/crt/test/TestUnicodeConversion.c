...
static BOOL test_win_convert_to_utf16(const testcase_t* test)
{
	...
	if (rc2 != wlen + 1)
	{
		...
		create_prefix(prefix, ARRAYSIZE(prefix), 0, rc2, -1, test, __func__, __LINE__);
		fprintf(stderr,
		        "%s MultiByteToWideChar(CP_UTF8, 0, %s, [-1], NULL, 0) expected %d, got %d\n",
...
int TestUnicodeConversion(int argc, char* argv[])
{
	...
	if (!test_conversion(testcases, ARRAYSIZE(testcases)))
		return -1;

#if defined(WITH_WINPR_DEPRECATED)
	if (!test_win_conversion(testcases, ARRAYSIZE(testcases)))
		return -1;
