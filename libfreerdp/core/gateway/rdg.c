...
static BOOL rdg_write_chunked(rdpRdg* rdg, wStream* sPacket)
{
	size_t len;
	int status;
	wStream* sChunk;
...
static BOOL rdg_write_websocket(rdpRdg* rdg, wStream* sPacket)
{
	size_t len;
	size_t fullLen;
	int status;
...
static BOOL rdg_write_packet(rdpRdg* rdg, wStream* sPacket)
{
	if (rdg->transferEncoding.isWebsocketTransport)
		return rdg_write_websocket(rdg, sPacket);

	return rdg_write_chunked(rdg, sPacket);
}
...
static int rdg_websocket_reply_close(BIO* bio)
{
	/* write back close */
	wStream* closeFrame;
	uint32_t maskingKey;
...
static int rdg_websocket_reply_pong(BIO* bio)
{
	wStream* pong;
	uint32_t maskingKey;
	int status;
	/* write pong back */
	pong = Stream_New(NULL, 6);
	Stream_Write_UINT8(pong, WEBSOCKET_FIN_BIT | WebsocketPongOpcode);
	Stream_Write_UINT8(pong, 0 | WEBSOCKET_MASK_BIT); /* no payload */
	winpr_RAND((BYTE*)&maskingKey, 4);
	Stream_Write_UINT32(pong, maskingKey); /* dummy masking key. */
	Stream_SealLength(pong);

	status = BIO_write(bio, Stream_Buffer(pong), Stream_Length(pong));
	Stream_Free(pong, TRUE);
