...
auth_status utils_authenticate_gateway(freerdp* instance, rdp_auth_reason reason)
{
	...
	if (!instance->GatewayAuthenticate && !instance->GatewayAuthenticateEx)
		return AUTH_NO_CREDENTIALS;

	if (instance->GatewayAuthenticateEx)
		proceed = instance->GatewayAuthenticateEx(instance, &settings->GatewayUsername,
		                                          &settings->GatewayPassword,
		                                          &settings->GatewayDomain, reason);
	else
		proceed =
