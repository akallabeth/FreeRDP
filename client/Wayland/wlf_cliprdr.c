...
static const char* mime_image[] = {
	...
	"image/gif",       "image/tiff",  "image/webp",         "image/png",
	"image/jpeg"
};
                                           ...
                                           const CLIPRDR_FORMAT_LIST* formatList)
{
	...
	if (image)
	{
		...
		{
			const char* mime = mime_image[x];
			const UINT32 fmtId = ClipboardGetFormatId(clipboard->system, mime);
			if (fmtId == 0)
				continue;
			UwacClipboardOfferCreate(clipboard->seat, mime);
		}
	}
