        ...
        SMARTCARD_OPERATION* operation)
{
	...
	if ((status = smartcard_unpack_establish_context_call(smartcard, irp->input,
	              call)))
	{
		WLog_ERR(TAG, "smartcard_unpack_establish_context_call failed with error %lu",
		         status);
		return status;
	}
...
static LONG smartcard_EstablishContext_Call(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation, EstablishContext_Call* call)
{
	LONG status;
	...
	if ((status = smartcard_pack_establish_context_return(smartcard, irp->output,
	              &ret)))
	{
		WLog_ERR(TAG, "smartcard_pack_establish_context_return failed with error %lu",
		         status);
		return status;
	}
...
static LONG smartcard_ReleaseContext_Call(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation, Context_Call* call)
{
	LONG status;
	Long_Return ret;
...
static LONG smartcard_ListReadersA_Call(SMARTCARD_DEVICE* smartcard,
                                        SMARTCARD_OPERATION* operation, ListReaders_Call* call)
{
	LONG status;
...
static LONG smartcard_GetStatusChangeA_Call(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation, GetStatusChangeA_Call* call)
{
	LONG status;
	UINT32 index;
...
static LONG smartcard_GetStatusChangeW_Call(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation, GetStatusChangeW_Call* call)
{
	LONG status;
	UINT32 index;
	GetStatusChange_Return ret;
...
static LONG smartcard_ConnectA_Call(SMARTCARD_DEVICE* smartcard,
                                    SMARTCARD_OPERATION* operation, ConnectA_Call* call)
{
	LONG status;
	SCARDHANDLE hCard = 0;
...
static LONG smartcard_ConnectW_Call(SMARTCARD_DEVICE* smartcard,
                                    SMARTCARD_OPERATION* operation, ConnectW_Call* call)
{
	LONG status;
	SCARDHANDLE hCard = 0;
	Connect_Return ret = { 0 };
...
static LONG smartcard_Reconnect_Call(SMARTCARD_DEVICE* smartcard,
                                     SMARTCARD_OPERATION* operation, Reconnect_Call* call)
{
	LONG status;
...
static LONG smartcard_Disconnect_Call(SMARTCARD_DEVICE* smartcard,
                                      SMARTCARD_OPERATION* operation, HCardAndDisposition_Call* call)
{
	LONG status;
	Long_Return ret;

	if ((status = ret.ReturnCode = SCardDisconnect(operation->hCard,
	                               call->dwDisposition)))
	{
        ...
        SMARTCARD_OPERATION* operation, HCardAndDisposition_Call* call)
{
	...
	if ((ret.ReturnCode = SCardEndTransaction(operation->hCard,
	                      call->dwDisposition)))
	{
...
static LONG smartcard_State_Call(SMARTCARD_DEVICE* smartcard,
                                 SMARTCARD_OPERATION* operation, State_Call* call)
{
	LONG status;
	State_Return ret;
	IRP* irp = operation->irp;
	ret.cbAtrLen = SCARD_ATR_LENGTH;
	ret.ReturnCode = SCardState(operation->hCard, &ret.dwState, &ret.dwProtocol,
...
static LONG smartcard_StatusA_Call(SMARTCARD_DEVICE* smartcard,
                                   SMARTCARD_OPERATION* operation, Status_Call* call)
{
	LONG status;
	Status_Return ret = { 0 };
...
static LONG smartcard_Transmit_Call(SMARTCARD_DEVICE* smartcard,
                                    SMARTCARD_OPERATION* operation, Transmit_Call* call)
{
	LONG status;
	Transmit_Return ret;
	IRP* irp = operation->irp;
	...
	ret.ReturnCode = SCardTransmit(operation->hCard, call->pioSendPci,
	                               call->pbSendBuffer,
...
static LONG smartcard_Control_Call(SMARTCARD_DEVICE* smartcard,
                                   SMARTCARD_OPERATION* operation, Control_Call* call)
{
	LONG status;
	Control_Return ret;
	IRP* irp = operation->irp;
...
static LONG smartcard_GetAttrib_Call(SMARTCARD_DEVICE* smartcard,
                                     SMARTCARD_OPERATION* operation, GetAttrib_Call* call)
{
	LONG status;
	DWORD cbAttrLen;
	BOOL autoAllocate;
...
static LONG smartcard_AccessStartedEvent_Call(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation, Long_Call* call)
{
...
static LONG smartcard_LocateCardsByATRA_Call(SMARTCARD_DEVICE* smartcard,
        SMARTCARD_OPERATION* operation, LocateCardsByATRA_Call* call)
{
	LONG status;
	BOOL equal;
	DWORD i, j, k;
        ...
        SMARTCARD_OPERATION* operation)
{
	    ...
	    (ioControlCode != SCARD_IOCTL_RELEASESTARTEDEVENT))
	{
		...
		{
			WLog_ERR(TAG, "smartcard_unpack_common_type_header failed with error %lu",
			         status);
			return SCARD_F_INTERNAL_ERROR;
		}
