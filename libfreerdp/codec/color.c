                                      ...
                                      gdiPalette* palette)
{
	...
	for (size_t i = 0; i < cbColorTable / 4; i++)
	{
    ...
    const gdiPalette* palette)
{
	...
	for (size_t y = 0; y < nHeight; y++)
	{
		...
		{
			if (andMask)
				andBits = &andMask[andStep * (nHeight - y - 1ull)];

			xorBits = &xorMask[xorStep * (nHeight - y - 1ull)];
		}
...
{
	const size_t dstByte = FreeRDPGetBytesPerPixel(DstFormat);
	const size_t srcByte = FreeRDPGetBytesPerPixel(SrcFormat);
	const size_t copyDstWidth = nWidth * dstByte;
	const size_t xSrcOffset = nXSrc * srcByte;
	const size_t xDstOffset = nXDst * dstByte;
		...
		for (size_t y = 0; y < nHeight; y++)
		{
			...
			for (size_t x = 1; x < nWidth; x++)
			{
				...
				else
				{
					...
					FreeRDPWriteColorIgnoreAlpha(&dstLine[1ull * (x + nXDst) * dstByte], DstFormat,
					                             dstColor);
				}
			}
		}
		...
		for (size_t y = 0; y < nHeight; y++)
		{
			...
			for (UINT32 x = 1; x < nWidth; x++)
			{
				...
				else
				{
					oldColor = color;
					dstColor = FreeRDPConvertColor(color, SrcFormat, DstFormat, palette);
					FreeRDPWriteColor(&dstLine[1ull * (x + nXDst) * dstByte], DstFormat, dstColor);
				}
			}
		}
	}
                                       ...
                                       UINT32 flags)
{
	...
	if (((flags & FREERDP_KEEP_DST_ALPHA) != 0) && FreeRDPColorHasAlpha(DstFormat))
	{
		for (size_t y = 0; y < nHeight; y++)
		{
			...
			UINT32 color = FreeRDPReadColor(&srcLine[1ull * nXSrc * srcByte], SrcFormat);
			UINT32 oldColor = color;
			UINT32 dstColor = FreeRDPConvertColor(color, SrcFormat, DstFormat, palette);
			FreeRDPWriteColorIgnoreAlpha(&dstLine[1ull * nXDst * dstByte], DstFormat, dstColor);
			for (size_t x = 1; x < nWidth; x++)
			{
				color = FreeRDPReadColor(&srcLine[(x + nXSrc) * srcByte], SrcFormat);
				if (color == oldColor)
	...
	else
	{
		for (size_t y = 0; y < nHeight; y++)
		{
			...
			UINT32 oldColor = color;
			UINT32 dstColor = FreeRDPConvertColor(color, SrcFormat, DstFormat, palette);
			FreeRDPWriteColor(&dstLine[1ull * nXDst * dstByte], DstFormat, dstColor);
			for (size_t x = 1; x < nWidth; x++)
			{
				color = FreeRDPReadColor(&srcLine[(x + nXSrc) * srcByte], SrcFormat);
				if (color == oldColor)
                        ...
                        UINT32 nXDst, UINT32 nYDst, UINT32 nWidth, UINT32 nHeight, UINT32 color)
{
	...
	for (size_t x = 0; x < nWidth; x++)
	{
		BYTE* pDst = &pFirstDstLine[(x + 1ull * nXDst) * bpp];
		FreeRDPWriteColor(pDst, DstFormat, color);
	}

	for (size_t y = 1; y < nHeight; y++)
	{
		BYTE* pDstLine = &pDstData[1ull * (y + nYDst) * nDstStep + 1ull * nXDst * bpp];
