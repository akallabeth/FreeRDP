...
static void* winpr_bitmap_write_buffer(const void* data, size_t size, UINT32 width, UINT32 height,
                          ...
                          size_t height, size_t bpp)
{
	...
	void* bmpdata = winpr_bitmap_write_buffer(width, height, stride, bpp, data, size, &bmpsize);
	if (!bmpdata)
		goto fail;
...
BOOL winpr_image_equal(const wImage* imageA, const wImage* imageB)
{
	...
	const size_t sizeA = imageA->scanline * imageA->height;
...
void* winpr_image_write_buffer(wImage* image, UINT32 format, size_t* psize)
{
	switch (format)
	{
		...
		{
			UINT32 outsize = 0;
			size_t size = image->height * image->scanline;
