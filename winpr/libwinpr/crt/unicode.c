...
SSIZE_T ConvertWCharToUtf8(const WCHAR* wstr, char* str, size_t len)
{
	...
	const int rc =
	    int_WideCharToMultiByte(CP_UTF8, 0, wstr, -1, str, (int)MIN(INT32_MAX, len), NULL, NULL);
	if (rc <= 0)
		return rc;
	else if (rc == len)
	{
...
SSIZE_T ConvertWCharNToUtf8(const WCHAR* wstr, size_t wlen, char* str, size_t len)
{
	...
	isNullTerminated = _wcsnlen(wstr, wlen) < wlen;
	const int rc = int_WideCharToMultiByte(CP_UTF8, 0, wstr, (int)wlen, str,
	                                       (int)MIN(INT32_MAX, len), NULL, NULL);
...
SSIZE_T ConvertUtf8ToWChar(const char* str, WCHAR* wstr, size_t wlen)
{
	...
	const int rc = int_MultiByteToWideChar(CP_UTF8, 0, str, -1, wstr, iwlen);
	if (rc <= 0)
		return rc;
	else if (iwlen == rc)
	{
...
SSIZE_T ConvertUtf8NToWChar(const char* str, size_t len, WCHAR* wstr, size_t wlen)
{
	...
	const int iwlen = MIN(INT32_MAX, wlen);
	const int rc = int_MultiByteToWideChar(CP_UTF8, 0, str, (int)len, wstr, (int)iwlen);
