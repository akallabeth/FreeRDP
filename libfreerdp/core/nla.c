...
static int nla_client_init(rdpNla* nla)
{
	...
	if (PromptPassword && settings->Username && strlen(settings->Username))
	{
		...
		if (sam)
		{
			entry = SamLookupUserA(sam, settings->Username, strlen(settings->Username),
			                       NULL, 0);
	...
	nla->status = nla->table->QuerySecurityPackageInfo(NLA_PKG_NAME,
	              &nla->pPackageInfo);
...
int nla_client_begin(rdpNla* nla)
{
	...
	if ((nla->status == SEC_I_COMPLETE_AND_CONTINUE)
	    || (nla->status == SEC_I_COMPLETE_NEEDED))
	{
	...
	winpr_HexDump(TAG, WLOG_DEBUG, nla->negoToken.pvBuffer,
...
static int nla_client_recv(rdpNla* nla)
{
	...
	if (nla->state == NLA_STATE_NEGO_TOKEN)
	{
		...
		nla->status = nla->table->InitializeSecurityContext(&nla->credentials,
		              ...
		              0, &nla->context, &nla->outputBufferDesc, &nla->pfContextAttr,
		              &nla->expiration);
		...
		if (nla->status == SEC_E_OK)
		{
			...
			nla->status = nla->table->QueryContextAttributes(&nla->context,
			              SECPKG_ATTR_SIZES, &nla->ContextSizes);
...
static int nla_server_init(rdpNla* nla)
{
...
#ifdef UNICODE
		pInitSecurityInterface = (INIT_SECURITY_INTERFACE) GetProcAddress(hSSPI,
	...
	nla->cbMaxToken = nla->pPackageInfo->cbMaxToken;
	nla->status = nla->table->AcquireCredentialsHandle(NULL, NLA_PKG_NAME,
	              SECPKG_CRED_INBOUND, NULL, NULL, NULL, NULL, &nla->credentials,
	              &nla->expiration);
...
static SECURITY_STATUS nla_encrypt_public_key_echo(rdpNla* nla)
{
	...
	if (!sspi_SecBufferAlloc(&nla->pubKeyAuth,
	                         nla->ContextSizes.cbSecurityTrailer + public_key_length))
	...
	Buffers[1].pvBuffer = ((BYTE*) nla->pubKeyAuth.pvBuffer) +
	...
	status = nla->table->EncryptMessage(&nla->context, 0, &Message,
	                                    nla->sendSeqNum++);
	...
	if (Buffers[0].cbBuffer < nla->ContextSizes.cbSecurityTrailer)
	{
		...
		MoveMemory(((BYTE*)nla->pubKeyAuth.pvBuffer) + Buffers[0].cbBuffer,
...
static SECURITY_STATUS nla_decrypt_public_key_echo(rdpNla* nla)
{
	...
	{
		WLog_ERR(TAG, "unexpected pubKeyAuth buffer size: %lu",
	...
	Message.ulVersion = SECBUFFER_VERSION;
	Message.pBuffers = (PSecBuffer) &Buffers;
	status = nla->table->DecryptMessage(&nla->context, &Message, nla->recvSeqNum++,
	                                    &pfQOP);
...
static int nla_sizeof_ts_password_creds(rdpNla* nla)
{
	int length = 0;
...
static int nla_sizeof_ts_csp_data_detail(rdpNla* nla)
{
	int length = 0;
...
static int nla_sizeof_ts_smartcard_creds(rdpNla* nla)
{
	int length = 0;
	...
	if (nla->identity)
	{
		/* TODO: PIN */
		length += ber_sizeof_sequence_octet_string(nla->identity->DomainLength * 2);
		length += nla_sizeof_ts_csp_data_detail(nla);
		// TODO: User Hint
		length += ber_sizeof_sequence_octet_string(nla->identity->DomainLength * 2);
		// TODO: domain Hint
		length += ber_sizeof_sequence_octet_string(nla->identity->DomainLength * 2);
...
static int nla_sizeof_ts_remote_guard_package_creds(rdpNla* nla)
{
	int length = 0;
...
static int nla_sizeof_ts_remote_guard_creds(rdpNla* nla)
{
	int length = 0;

	if (nla->identity)
	{
		length += nla_sizeof_ts_remote_guard_package_creds(nla);
		// TODO: Optional creds
	}
...
{
	int length, ts_password_creds_length;
	...
	if (!ber_read_octet_string_tag(s, &ts_password_creds_length))
		return FALSE;
	...
	if (nla->identity->DomainLength > 0)
	{
		...
		CopyMemory(nla->identity->Domain, Stream_Pointer(s),
		           nla->identity->DomainLength);
	...
	if (nla->identity->PasswordLength > 0)
	{
		...
		CopyMemory(nla->identity->Password, Stream_Pointer(s),
...
static BOOL nla_read_ts_smartcard_creds(rdpNla* nla, wStream* s)
{
	...
	if (!ber_read_octet_string_tag(s, &creds_length))
		return FALSE;
...
static int nla_write_ts_csp_data_detail(rdpNla* nla, wStream* s)
{
	int size = 0;
	int innerSize = nla_sizeof_ts_csp_data_detail(nla);
	/* TSCspDataDetail (SEQUENCE) */
	size += ber_write_sequence_tag(s, innerSize);
	/* [0] keySpec (INTEGER) */
	...
	size += ber_write_sequence_octet_string(
	            s, 2, (BYTE*) nla->identity->User,
	            nla->identity->UserLength * 2);
	/* [3] containerName (OCTET STRING OPTIONAL) */
	size += ber_write_sequence_octet_string(
	            s, 3, (BYTE*) nla->identity->User,
	            nla->identity->UserLength * 2);
	/* [4] cspName (OCTET STRING OPTIONAL) */
	size += ber_write_sequence_octet_string(
	            s, 4, (BYTE*) nla->identity->User,
	            nla->identity->UserLength * 2);
	return size;
}

static int nla_write_ts_smartcard_creds(rdpNla* nla, wStream* s)
{
	// TODO
	int size = 0;
	const int innerSize = nla_sizeof_ts_smartcard_creds(nla);
	/* TSSmartCardCreds (SEQUENCE) */
	size += ber_write_sequence_tag(s, innerSize);
...
static BOOL nla_read_ts_remote_guard_creds(rdpNla* nla, wStream* s)
{
	int length, creds_length;
...
static int nla_write_ts_remote_guard_package_creds(rdpNla* nla, wStream* s)
{
	int size = 0;
	int innerSize = nla_sizeof_ts_remote_guard_package_creds(nla);
	/* TSRemoteGuardPackageCreds (SEQUENCE) */
	size += ber_write_sequence_tag(s, innerSize);
	/* [0] packageName (OCTET STRING) */
	size += ber_write_sequence_octet_string(
	            s, 0, (BYTE*) nla->identity->Domain,
...
static int nla_write_ts_remote_guard_creds(rdpNla* nla, wStream* s)
{
	int size = 0;
	int innerSize = nla_sizeof_ts_remote_guard_creds(nla);
	/* TSRemoteGuardPackageCreds (SEQUENCE) */
	size += ber_write_sequence_tag(s, innerSize);
	...
	if (nla->identity)
	{
		...
		size += nla_write_ts_remote_guard_package_creds(nla, s);
...
static int nla_sizeof_ts_credentials(rdpNla* nla)
{
	...
	size += ber_sizeof_sequence_octet_string(ber_sizeof_sequence(
	            nla_sizeof_ts_password_creds(nla)));
	return size;
}
...
static BOOL nla_read_ts_credentials(rdpNla* nla, PSecBuffer ts_credentials)
{
	...
	switch (type)
	{
		...
		case 6:
			ret = nla_read_ts_remote_guard_creds(nla, s);
			break;
...
static int nla_write_ts_credentials(rdpNla* nla, wStream* s)
{
	...
	size += ber_write_contextual_tag(s, 1, ber_sizeof_octet_string(passwordSize),
	                                 TRUE);
	size += ber_write_octet_string_tag(s, passwordSize);

	if (nla->settings->SmartCardAuth)
		size += nla_write_ts_smartcard_creds(nla, s);
	else if (nla->settings->RemoteGuardAuth)
		size += nla_write_ts_remote_guard_creds(nla, s);
	else
...
static SECURITY_STATUS nla_encrypt_ts_credentials(rdpNla* nla)
{
	...
	CopyMemory(Buffers[1].pvBuffer, nla->tsCredentials.pvBuffer,
	           Buffers[1].cbBuffer);
	Message.cBuffers = 2;
	Message.ulVersion = SECBUFFER_VERSION;
	Message.pBuffers = (PSecBuffer) &Buffers;
	...
	{
		/* EncryptMessage may not use all the signature space, so we need to shrink the excess */
		MoveMemory(((BYTE*)nla->authInfo.pvBuffer) + Buffers[0].cbBuffer,
		           Buffers[1].pvBuffer, Buffers[1].cbBuffer);
...
static BOOL nla_send(rdpNla* nla)
{
	...
	if (nla->version < 3 || nla->errorCode == 0)
	{
		...
		auth_info_length = (nla->authInfo.cbBuffer > 0) ? nla_sizeof_auth_info(
		                       nla->authInfo.cbBuffer) : 0;
	}
	else
	{
	...
	length = nego_tokens_length + pub_key_auth_length + auth_info_length +
	         error_code_context_length + error_code_length;
	...
	if (nego_tokens_length > 0)
	{
		...
		length -= ber_write_contextual_tag(s, 1,
		                                   ber_sizeof_sequence(ber_sizeof_sequence(ber_sizeof_sequence_octet_string(
		                                           nla->negoToken.cbBuffer))), TRUE); /* NegoData */
	...
	if (auth_info_length > 0)
	{
		...
		length -= ber_write_sequence_octet_string(s, 2, nla->authInfo.pvBuffer,
	...
	if (pub_key_auth_length > 0)
	{
		...
		length -= ber_write_sequence_octet_string(s, 3, nla->pubKeyAuth.pvBuffer,
		          nla->pubKeyAuth.cbBuffer);
	}
...
static void nla_buffer_print(rdpNla* nla)
{
	...
	if (nla->pubKeyAuth.cbBuffer > 0)
	{
		...
		winpr_HexDump(TAG, WLOG_DEBUG, nla->pubKeyAuth.pvBuffer,
		              nla->pubKeyAuth.cbBuffer);
	}
...
LPTSTR nla_make_spn(const char* ServiceClass, const char* hostname)
{
	...
	status = DsMakeSpn(ServiceClassX, hostnameX, NULL, 0, NULL, &SpnLength,
...
rdpNla* nla_new(freerdp* instance, rdpTransport* transport,
