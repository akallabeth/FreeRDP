...
static BOOL udevman_loading_lock(IUDEVMAN* idevman)
{
	...
	return WaitForSingleObject(udevman->devman_loading, INFINITE) == WAIT_OBJECT_0;
}
...
static BOOL udevman_wait_urb(IUDEVMAN* idevman)
{
	...
	return WaitForSingleObject(udevman->sem_urb_lock, INFINITE) == WAIT_OBJECT_0;
}
...
int freerdp_urbdrc_client_subsystem_entry(PFREERDP_URBDRC_SERVICE_ENTRY_POINTS pEntryPoints)
{
	...
	udevman->devman_loading = CreateMutexA(NULL, FALSE, "udevman->devman_loading");
	if (!udevman->devman_loading)
		goto fail;

	udevman->sem_urb_lock = CreateSemaphoreA(NULL, MAX_URB_REQUSET_NUM,
	                                         MAX_URB_REQUSET_NUM, "udevman->sem_urb_lock");
	if (!udevman->sem_urb_lock)
		goto fail;
