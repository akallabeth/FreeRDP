...

{
	if (in_count > 0)
	{
		...
		Stream_Write(in_s, Stream_Buffer(in_data), in_count * 2ull);
	}
...
static INLINE UINT16 out_copy_count_3(UINT16 in_count, wStream* in_s, wStream* in_data)
{
	if (in_count > 0)
	{
		...
		Stream_Write(in_s, Stream_Pointer(in_data), in_count * 3ull);
	}
                                          ...
                                          wStream* temp_s, UINT32 e)
{
	...
	{
		size_t i = Stream_GetPosition(s) + count * 3ull;

		if ((i - (color_count * 3ull) >= byte_limit) &&
		    (i - (bicolor_count * 3ull) >= byte_limit) && (i - (fill_count * 3ull) >= byte_limit) &&
		    (i - (mix_count * 3ull) >= byte_limit) && (i - (fom_count * 3ull) >= byte_limit))
		{
			break;
		}
                                          ...
                                          UINT32 start_line, wStream* temp_s, UINT32 e)
{
	...
	while ((line >= start) && (out_count < 32768))
	{
		size_t i = Stream_GetPosition(s) + count * 2ull;

		if ((i - (color_count * 2ull) >= byte_limit) &&
		    (i - (bicolor_count * 2ull) >= byte_limit) && (i - (fill_count * 2ull) >= byte_limit) &&
		    (i - (mix_count * 2ull) >= byte_limit) && (i - (fom_count * 2ull) >= byte_limit))
		{
			break;
		}
