 ...
 * Image Processing using FFmpeg (Runtime Loading)
 *
...
#define TAG FREERDP_TAG("codec.image")

typedef struct SwsContext* (*pSws_getContext)(int srcW, int srcH, int srcFormat, int dstW, int dstH,
...
#if defined(LIBSWSCALE_VERSION_MAJOR)
#define SWSCALE_VERSION_STR TOSTRING(LIBSWSCALE_VERSION_MAJOR)
#endif

static const char* swscale_library_names[] = {
#if defined(_WIN32)
#if defined(SWSCALE_VERSION_STR)
	"swscale-" SWSCALE_VERSION_STR ".dll",
#endif
	"swscale-9.dll", "swscale-8.dll", "swscale-7.dll", "swscale-6.dll", "swscale.dll"
#elif defined(__APPLE__)
#if defined(SWSCALE_VERSION_STR)
	"libswscale." SWSCALE_VERSION_STR ".dylib",
#endif
	"libswscale.dylib", "libswscale.9.dylib", "libswscale.8.dylib", "libswscale.7.dylib",
	"libswscale.6.dylib"
#else
#if defined(SWSCALE_VERSION_STR)
	"libswscale.so." SWSCALE_VERSION_STR,
#endif
...
static BOOL swscale_load_library(const char* name)
{
	...
	g_swscale.getContext = (pSws_getContext)(void*)GetProcAddress(g_swscale.lib, "sws_getContext");
...
BOOL freerdp_swscale_init(void)
{
	...
	WLog_INFO(TAG, "swscale library not found");
...
#define AVUTIL_VERSION_STR TOSTRING(LIBAVUTIL_VERSION_MAJOR)
#endif

static const char* avutil_library_names[] = {
#if defined(_WIN32)
#if defined(AVUTIL_VERSION_STR)
	"avutil-" AVUTIL_VERSION_STR ".dll",
#endif
	"avutil-59.dll", "avutil-58.dll", "avutil-57.dll", "avutil-56.dll", "avutil.dll"
#elif defined(__APPLE__)
#if defined(AVUTIL_VERSION_STR)
	"libavutil." AVUTIL_VERSION_STR ".dylib",
#endif
	"libavutil.dylib", "libavutil.59.dylib", "libavutil.58.dylib", "libavutil.57.dylib",
	"libavutil.56.dylib"
#else
#if defined(AVUTIL_VERSION_STR)
	"libavutil.so." AVUTIL_VERSION_STR,
#endif
...
BOOL freerdp_avutil_init(void)
{
	...
	WLog_INFO(TAG, "avutil library not found");
