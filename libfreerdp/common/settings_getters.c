...
static BOOL check_readonly(const rdpSettings* settings)
{
	const rdpSettingsInternal* s = freerdp_settings_intern_const_cast(settings);
	WINPR_ASSERT(s);
	if (s->readonly)
	{
		WLog_ERR(TAG, "this settings copy is readonly, access denied");
		return FALSE;
	}
	return TRUE;
}
