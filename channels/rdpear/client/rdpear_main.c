...
/* defined in libkrb5 */
extern krb5_error_code encode_krb5_authenticator(const krb5_authenticator* rep,
                                                 krb5_data** code_out);
extern krb5_error_code encode_krb5_ap_rep(const krb5_ap_rep* rep, krb5_data** code_out);
...
static krb5_error_code kerb_do_checksum(krb5_context ctx, KERB_RPC_ENCRYPTION_KEY* key,
                                        krb5_keyusage kusage, krb5_cksumtype cksumtype,
                                        KERB_ASN1_DATA* plain, krb5_checksum* out)
{
	WINPR_ASSERT(ctx);
	WINPR_ASSERT(key);
	WINPR_ASSERT(plain);
...
static krb5_error_code kerb_do_encrypt(krb5_context ctx, KERB_RPC_ENCRYPTION_KEY* key,
                                       krb5_keyusage kusage, KERB_ASN1_DATA* plain, krb5_data* out)
{
	WINPR_ASSERT(ctx);
	WINPR_ASSERT(key);
	WINPR_ASSERT(plain);
...
static krb5_error_code kerb_do_decrypt(krb5_context ctx, KERB_RPC_ENCRYPTION_KEY* key,
                                       krb5_keyusage kusage, krb5_data* cipher,
                                       KERB_ASN1_DATA* plain)
{
	...
	if (data.length > 0)
		data.data = malloc(cipher->length);
	if (!data.data)
	{
		rv = ENOMEM;
		goto out;
                                ...
                                RdpEarPackageType packageType, wStream* payload)
{
	...
	if (!unencodedContent || !finalStream)
		goto out;
                                               ...
                                               KERB_ASN1_DATA* asn1)
{
	...
	*pstatus = ERROR_INVALID_DATA;
	...
	krb5_error_code rv = kerb_do_encrypt(rdpear->krbContext, req.Key, (krb5_keyusage)req.KeyUsage,
...
static BOOL extractAuthData(const KERB_ASN1_DATA* src, krb5_authdata* authData, BOOL* haveData)
{
	...
	WINPR_ASSERT(src);
	WINPR_ASSERT(authData);
	WINPR_ASSERT(haveData);
...
static INT64 krb5_time_to_FILETIME(const krb5_timestamp* ts, krb5_int32 usec)
{
	WINPR_ASSERT(ts);
	WINPR_ASSERT(usec >= 0);
...
static void krb5_free_principal_contents(krb5_context ctx, krb5_principal principal)
{
	...
	krb5_free_data_contents(ctx, principal->data);
}
                                                 ...
                                                 CreateApReqAuthenticatorResp* resp)
{
	...
	*pstatus = ERROR_INVALID_DATA;
	...
	krb5_authenticator authent = { 0 };
	authent.seq_number = req.SequenceNumber;
	authent.authorization_data = haveData ? authDataPtr : NULL;

	client.type = req.ClientName->NameType;
	WINPR_ASSERT(req.ClientName->nameHints.count <= INT32_MAX);
	...
	rv = krb5_us_timeofday(rdpear->krbContext, &authent.ctime, &authent.cusec);
	if (rv)
		goto out;
	...
	KERB_ASN1_DATA plain_authent = { 0 };
	plain_authent.Asn1Buffer = (BYTE*)der->data;
	plain_authent.Asn1BufferHints.count = der->length;
                                           ...
                                           UnpackKdcReplyBodyResp* resp)
{
	...
	*pstatus = ERROR_INVALID_DATA;
	...
	int encType = 0;
	if (!rdpear_findEncryptedData(req.EncryptedData, &encType, &asn1Data))
		goto out;
                                       ...
                                       KERB_ASN1_DATA* resp)
{
	...
	*pstatus = ERROR_INVALID_DATA;
	...
	krb5_data asn1Data = { 0 };
	int encType = 0;
	if (!rdpear_findEncryptedData(req.EncryptedReply, &encType, &asn1Data))
		goto out;
                                  ...
                                  RdpEarPackageType packageType, wStream* s)
{
	...
	WINPR_ASSERT(rdpear);
	WINPR_ASSERT(pChannelCallback);
	WINPR_UNUSED(packageType);

	wStream* respStream = Stream_New(NULL, 4096);
	if (!respStream)
		goto out;
	...
	/* TODO :adjust gotos to more precise cleanups */
...
static UINT rdpear_on_data_received(IWTSVirtualChannelCallback* pChannelCallback, wStream* s)
{
	...
	SecBuffer decrypted = { Length, SECBUFFER_DATA, NULL };
