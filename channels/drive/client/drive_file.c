...
static BOOL drive_file_set_fullpath(DRIVE_FILE* file, const WCHAR* base_path, const WCHAR* filename)
{
	WINPR_ASSERT(file->backend);
	WINPR_ASSERT(file->context);
	WINPR_ASSERT(file->backend->setPath);
	return file->backend->setPath(file->context, base_path, filename);
}
...
static BOOL drive_file_init(DRIVE_FILE* file)
{
	...
	DWORD dwAttr = file->backend->getFileAttributes(file->context);
                           ...
                           UINT32 CreateOptions, UINT32 FileAttributes, UINT32 SharedAccess)
{
	...
	if (!drive_file_set_fullpath(file, base_path, path))
		goto fail;
...
BOOL drive_file_seek(DRIVE_FILE* file, UINT64 Offset)
{
	...
	const SSIZE_T rc = file->backend->seek(file->context, (SSIZE_T)Offset, SEEK_SET);
	return rc == (SSIZE_T)Offset;
}
...
BOOL drive_file_read(DRIVE_FILE* file, BYTE* buffer, UINT32* Length)
{
	...
	const SSIZE_T rc = file->backend->read(file->context, buffer, *Length);
	if (rc < 0)
		return FALSE;
	if (rc > *Length)
		return FALSE;
                                ...
                                wStream* input)
{
	...
	switch (FsInformationClass)
	{
		...
		case FileRenameInformation:
			...
			if (!file->backend->move(file->context, Stream_ConstPointer(input),
                                ...
                                const WCHAR* path, UINT32 PathWCharLength, wStream* output)
{
	...
	const WIN32_FIND_DATAW* current = NULL;
	if (InitialQuery != 0)
	{
		WINPR_ASSERT(file->backend->first);
		current = file->backend->first(file->context, path, PathWCharLength);
	}
	else
	{
		WINPR_ASSERT(file->backend->next);
		current = file->backend->next(file->context);
	}

	if (!current)
		goto out_fail;

	const size_t length = _wcslen(current->cFileName) * sizeof(WCHAR);
	...
	switch (FsInformationClass)
	{
		case FileDirectoryInformation:
			...
			Stream_Write_UINT32(output, current->ftCreationTime.dwLowDateTime);  /* CreationTime */
			Stream_Write_UINT32(output, current->ftCreationTime.dwHighDateTime); /* CreationTime */
			Stream_Write_UINT32(output,
			                    current->ftLastAccessTime.dwLowDateTime); /* LastAccessTime */
			Stream_Write_UINT32(output,
			                    current->ftLastAccessTime.dwHighDateTime); /* LastAccessTime */
			Stream_Write_UINT32(output, current->ftLastWriteTime.dwLowDateTime); /* LastWriteTime */
			Stream_Write_UINT32(output,
			                    current->ftLastWriteTime.dwHighDateTime);        /* LastWriteTime */
			Stream_Write_UINT32(output, current->ftLastWriteTime.dwLowDateTime); /* ChangeTime */
			Stream_Write_UINT32(output, current->ftLastWriteTime.dwHighDateTime); /* ChangeTime */
			Stream_Write_UINT32(output, current->nFileSizeLow);                   /* EndOfFile */
			Stream_Write_UINT32(output, current->nFileSizeHigh);                  /* EndOfFile */
			Stream_Write_UINT32(output, current->nFileSizeLow);            /* AllocationSize */
			Stream_Write_UINT32(output, current->nFileSizeHigh);           /* AllocationSize */
			Stream_Write_UINT32(output, current->dwFileAttributes);        /* FileAttributes */
			Stream_Write_UINT32(output, (UINT32)length);                   /* FileNameLength */
			Stream_Write(output, current->cFileName, length);
			break;
