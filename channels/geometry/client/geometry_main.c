...
static UINT32 mappedGeometryHash(const void* arg)
{
	const UINT64* g = (const UINT64*)arg;
	return (UINT32)((*g >> 32) + (*g & 0xffffffff));
}

static BOOL mappedGeometryKeyCompare(const void* arg1, const void* arg2)
{
	const UINT64* g1 = (const UINT64*)arg1;
	const UINT64* g2 = (const UINT64*)arg2;
	return *g1 == *g2;
}
...
UINT DVCPluginEntry(IDRDYNVC_ENTRY_POINTS* pEntryPoints)
{
	...
	if (!geometry)
	{
		...
		{
			wObject* keyObj = HashTable_KeyObject(context->geometries);
			wObject* valObj = HashTable_ValueObject(context->geometries);

			if (!keyObj || !valObj)
				goto error_context;

			keyObj->fnObjectEquals = mappedGeometryKeyCompare;
			valObj->fnObjectFree = mappedGeometryUnref;
		}
